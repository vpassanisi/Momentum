...
const mutations: MutationTree<CommentState> = {
  setComments: (state, obj: SetCommentsObj) => {
    ...
    if (obj.sort === "points") {
      state.pagination.lastVal = state.comments[obj.post._id][lastIndex].points;
    } else {
  ...
  getNextCommentsSuccess: (
    ...
    // update comments
    state.comments[postID] = state.comments[postID].concat(newComments[postID]);
...
const module: Module<CommentState, RootState> = {
  namespaced: true,
  state,
  actions,
  mutations,
